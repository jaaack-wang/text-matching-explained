    ...
    def _train(self, dataset, one_hot_y=True):
        ...
        for batch in dataset:
            ...
            if self.include_seq_len:
                preds = self.model(batch[0], batch[1], batch[2], batch[3])
            else:
...
class TextVectorizer:
    ...
    def __init__(self, tokenizer=None):
        ...
        else get_tokenizer('basic_english')
        self.vocab_to_idx = {}
        self.idx_to_vocab = {}
        self._V = None
...
def _batchify_fn(batch, 
    ...
    for (text_a, text_b, label) in batch:
        out_a.append(text_encoder(text_a)) 
        out_b.append(text_encoder(text_b)) 
        out_l.append(label)
    ...
    # hence, their text_seq_len is also reduced to the max_seq_len
        if max_seq_len:
            a_len = to_tensor([len(a) if len(a) < max_seq_len 
                               else max_seq_len for a in out_a])
            b_len = to_tensor([len(b) if len(b) < max_seq_len 
                               else max_seq_len for a in out_b])
        else:
            a_len = to_tensor([len(a) for a in out_a])
            b_len = to_tensor([len(b) for b in out_b])
        
    # torch.cat put a list of tensor together
    out_a = to_tensor(pad(out_a))
    out_b = to_tensor(pad(out_b))
    out_l = to_tensor(out_l)
    
    if include_seq_len:
        return out_a, out_b, a_len, b_len, out_l
    
    return out_a, out_b, out_l
